...
static void disassemble_xfer( char* str, unsigned int inst )
{
	static const char* bytenum[] = { "", ".b1", ".b2", ".b3" };
	unsigned int Rx, RxSel, Imm, XfrId, RWMode;

	Rx = (inst & 0x0000001F);
	RxSel = (inst & 0x00000060) >> 5;
	Imm = (inst & 0x00003F80) >> 7;
	XfrId = (inst & 0x007F8000) >> 15;
	RWMode = (inst & 0x01800000) >> 23;
	...
	if( XfrId == 254 ) {
		...
		case 11:
		case 12:
			sprintf( str, " SCRATCHPAD_%d,", XfrId - 10 );
			break;

		case 14:
			sprintf( str, " SCRATCHPAD_PRU," );
			break;
	...
	if( Imm < 124 ) sprintf( str, " R%u%s, %u", Rx, bytenum[RxSel], Imm + 1 );
	else sprintf( str, " R%u%s, b%u", Rx, bytenum[RxSel], Imm - 124 );
}
...
void disassemble(char *str, unsigned int inst)
{
    ...
    char			*f2_inst[] = {
                ...
                "RESERVED", "RESERVED", "LOOP", "RESERVED", "RESERVED", "RESERVED", "RESERVED", "RESERVED", "RESERVED",
                "SLP"
                };
        ...
        case 1:		// format 2
            ...
            switch (SUBOP) {
                ...
                case 4:  // SCAN
                    ...
                    if (IO) {
                        sprintf(str, "%s R%u%s, 0x%04x", f2_inst[SUBOP], Rd, sis[RdSel], Rs1 ); //Todo: , sis[Rs1Sel], Imm2);
                    } else {
                        sprintf(str, "%s R%u%s, R%u%s", f2_inst[SUBOP], Rd, sis[RdSel], Rs2, sis[Rs2Sel]);
                    }
                ...
                case 7: // XFER
                	disassemble_xfer( str, inst );
                	break;

                case 8: // LOOP
                	Rd = (inst & 0x001F0000) >> 16;
                	RdSel = (inst & 0x00E00060) >> 21;
                	Imm = (inst & 0x00007FFF);
                	Imm2 = (inst & 0x00FF0000) >> 16;

                	if( ((inst & 0x01000000) >> 23) == 0 ) {
                		sprintf( str, "%s%s %hu, R%u%s",
                				 ((inst & 0x00008000) >> 15) ? "I" : "",
                				 f2_inst[SUBOP], Imm, Rd, sis[RdSel] );
                	}
                	else {
                		sprintf( str, "%s%s %u, %u",
                				 ((inst & 0x00008000) >> 15) ? "I" : "",
                				 f2_inst[SUBOP], Imm, (unsigned int)Imm2 + 1 );
                	}
                	break;
