...
extern void printhelpbrief(void);
...
int main(int argc, char *argv[])
{
    ...
    unsigned int i;
    unsigned int bpnum, offset, wanum, value;
    int addr, len;
    int opt;
    ...
    while ((opt = getopt(argc, argv, "?a:p:um")) != -1) {
        switch (opt) {
            case 'a':
                opt_pruss_addr = strtoul(optarg, NULL, 0);
                break;
            ...
            default: /* '?' */
                ...
                printf("    -u - force the use of PRUSS device to map PRU memory space\n");
#else
                printf("    -u - force the use of UIO to map PRU memory space\n");
#endif
                printf("    -m - force the use of /dev/mem to map PRU memory space\n");
                printf("    if neither the -u or -m options are used then it will try the pruss-device first\n");
...
#if defined( __FreeBSD__ )
        snprintf( uio_dev_file, sizeof( uio_dev_file ), "%s", "/dev/pruss0" );
#else
    ...
    do {
        ...
        if (!strcmp(cmd, "?") || !strcmp(cmd, "HELP")) { // HELP - help command
            ...
            if (numargs == 0) {
                ...
                bpnum = strtol(&cmdargs[argptrs[0]], NULL, 0);
                addr = strtoul(&cmdargs[argptrs[1]], &str_end, 0);
                if (bpnum < MAX_BREAKPOINTS) {
                    if( *str_end == '\0' || isspace( *str_end ) ) cmd_set_breakpoint (bpnum, addr);
                    else cmd_set_breakpointdbg (bpnum, &cmdargs[argptrs[1]]);
                } else {
                    printf("ERROR: breakpoint number must be equal to or between 0 and %u\n", MAX_BREAKPOINTS-1);
                }
            } else {
            ...
            if (numargs > 2) {
                ...
                if (numargs == 2) {
                    addr = strtoul(&cmdargs[argptrs[0]], NULL, 0);
        ...
        else if (!strcmp(cmd, "DIS") || !strcmp(cmd, "DISDBG")) { // DIS - disassemble command
            if (numargs > 2) {
                printf("ERROR: too many arguments\n");
            } else {
                if (numargs == 2) {
                ...
                if ((addr < 0) || (addr > MAX_PRU_MEM - 1) || (len < 0) || (addr+len > MAX_PRU_MEM)) {
                    ...
                    last_len = len;
                    printf ("Absolute addr = 0x%04x, offset = 0x%04x, Len = %u\n", addr + offset, addr, len);
                    if( cmd[3] == '\0' ) cmd_dis(offset, addr, len);
                    else cmd_disdbg(offset, addr, len);
                }
            }
        }
...
//              cmd_run_at(addr);
        ...
        else if (!strcmp(cmd, "GB")) { // GB - Start program in background
            last_cmd = LAST_CMD_GB;
            if (numargs > 0) {
                printf("ERROR: too many arguments\n");
            } else {
                // start processor
        ...
        else if (!strcmp(cmd, "GSS")) { // GSS - Start program using single stepping to provide Watchpoints
            last_cmd = LAST_CMD_GSS;
            if (numargs > 0) {
                printf("ERROR: too many arguments\n");
            } else {
                // start processor
            ...
            if (numargs != 2) {
                ...
                for( int i = 0; i < MAX_BREAKPOINTS; i++ ) {
                    cmd_clear_breakpoint( i );
                }
                cmd_loadprog(addr, &cmdargs[argptrs[1]]);
            }
        }

        else if (!strcmp(cmd, "LDBG")) { // LDBG - Load PRU program
            last_cmd = LAST_CMD_NONE;
            if (numargs != 1) {
                printf("ERROR: incorrect number of arguments\n");
            } else {
            ...
            if (numargs == 0) {
                ...
                addr = strtoul(&cmdargs[argptrs[0]], NULL, 0);
                value = strtoul(&cmdargs[argptrs[1]], NULL, 0);
                cmd_setreg( addr, value );
            } else {
                printf("ERROR: incorrect number of arguments\n");
            }
        }
            ...
            if (numargs == 0) {
                ...
                wanum = strtol(&cmdargs[argptrs[0]], NULL, 0);
                addr = strtoul(&cmdargs[argptrs[1]], NULL, 0);
                if (wanum < MAX_WATCH) {
                ...
                wanum = strtol(&cmdargs[argptrs[0]], NULL, 0);
                addr = strtoul(&cmdargs[argptrs[1]], NULL, 0);
                value = strtoul(&cmdargs[argptrs[2]], NULL, 0);
                if (wanum < MAX_WATCH) {
